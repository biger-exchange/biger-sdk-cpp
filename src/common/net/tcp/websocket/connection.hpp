...
**    Author        : bd@biger.in
**    File          : connection.hpp
**    Date          : 2018-10-16 16:07:22
**    Description   :
**
****************************************************************************/

#ifndef BIGER_COMMON_NET_TCP_WEBSOCKET_CONNECTION_HPP
#define BIGER_COMMON_NET_TCP_WEBSOCKET_CONNECTION_HPP
...
class Connection
{
 public:
  ...
  {
    return ws_.lowest_layer().is_open();
  }
 ...
 protected:
  ...
  {
    if (!ws_.lowest_layer().is_open())
    {
      return;
    }

    ws_.lowest_layer().cancel(ec);
    if (ec)
    {
      BOOST_LOG_SEV(ucex_lg, error)
